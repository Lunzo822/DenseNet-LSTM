...
from keras.layers import Input, ZeroPadding1D, LSTM, Multiply, SeparableConv1D
...
from data_process import load_data, load_test_data
...
def conv_block(x, filters, dropout_rate=None):
    ...
    # x = SeparableConv1D(filters=filters, kernel_size=3, use_bias=False)(x)
    x = Activation('relu')(x)
...
def transition_block(x, out_fliters):
    ...
    # x = SeparableConv1D(out_fliters, 1, 1, use_bias=False)(x)
...
def dense_example(input_shape, classes):
    ...
    # x = SeparableConv1D(filters=2, kernel_size=3)(x)
...
def train_test():
    ...
    test_data, test_label = load_test_data(sub)
    ...
    EarlyStopping1 = callbacks.EarlyStopping(monitor='val_accuracy', patience=10),
    ...
    CHECK_ROOT = f'Result/{sub}/checkpoint{str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))}/'
    ...
    TensorBoard = callbacks.TensorBoard(log_dir=f'Result/{sub}/logs')
    ...
    total_size = total_params * 4 / (1024 ** 2)  # 4 bytes/param, 1024^2 = 1MB
    print("Total model size: {:.2f} MB".format(total_size))

    with open(f'Result/{sub}/config_{str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))}.txt', 'w') as f:
        print(confusion_matrix, '\n',
              ...
              'Total model size: {:.2f} MB'.format(total_size), '\n',
